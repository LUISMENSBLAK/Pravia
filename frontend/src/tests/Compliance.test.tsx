...
import {
  ...
  fixtureComplianceDirectory,
} from "../features/compliance/compliance.fixtures";
...
const permissions = [
  ...
  "compliance.notice.prepare",
  "compliance.sensitive.read",
  "expedientes.read",
  "documentos.read",
  "ai.use",
  "ai.cumplimiento.read",
];
const catalogs = {
  reglas: [
    {
  ...
  expedientes: [
    {
      id: "fixture-exp-141",
      numero_pravia: "EXP-2026-0141",
      cliente_alias: "María Fernanda López",
      tipo_acto: { nombre: "Compraventa" },
      notaria: { id: "not-12", numero_notaria: "12" },
    },
  ],
      ...
      if (/\/cumplimiento\/revisiones\/fixture-uif$/.test(url)) {
        ...
        const detail = options.redacted
          ? {
              ...fixtureComplianceDetail,
              workspace: {
                ...fixtureComplianceDetail.workspace!,
                parties: [],
                beneficialOwners: [],
                pepReviews: [],
                screenings: [],
        ...
        return json({
          ...
          ...(options.empty
            ? {
                ...fixtureComplianceDirectory,
                revisiones: [],
                meta: { ...fixtureComplianceDirectory.meta, total: 0 },
              }
            : fixtureComplianceDirectory),
        });
      }
...
describe("Riesgos / UIF canónico", () => {
  ...
  it("abre un workspace único con las diez secciones trazables", async () => {
    mockApi();
    renderAt("/riesgos/revisiones/fixture-uif");
    expect(
      await screen.findByRole("heading", { name: "EXP-2026-0141" }),
    ).toBeInTheDocument();
    for (const title of [
      "Operación y snapshot",
      "Actividad vulnerable",
      "Personas y comparecientes",
      "Beneficiario controlador",
      "PEP y listas",
      "Formas de pago y restricción de efectivo",
      "Evaluación interna de riesgo",
      "Obligaciones y Avisos",
      "Evidencia y conservación",
      "Historial, versiones e integraciones",
    ])
  ...
  it("separa Aviso, restricción de efectivo y riesgo interno", async () => {
    mockApi();
    renderAt("/riesgos/revisiones/fixture-uif");
    await screen.findByText("Actividad vulnerable identificada");
    expect(screen.getAllByText("Requiere Aviso").length).toBeGreaterThan(0);
    expect(
      screen.getByText(
        /Esta información histórica no determina restricciones actuales/,
      ),
    ).toBeInTheDocument();
    expect(screen.queryByText(/8,025 UMA/)).not.toBeInTheDocument();
    expect(
      screen.getByText(/no constituye una probabilidad de ilicitud/i),
    ).toBeInTheDocument();
  });
  ...
  it("no simula PEP y conserva la compuerta humana", async () => {
    ...
    renderAt("/riesgos/revisiones/fixture-uif");
    expect(
      (await screen.findAllByText("Consulta oficial PEP no configurada"))
        .length,
    ).toBeGreaterThan(0);
    expect(
      screen.getByText(
        /La coincidencia textual no genera por sí sola un Aviso de 24 horas/,
      ),
    ).toBeInTheDocument();
    expect(
      screen.getAllByText("PROPUESTA — REQUIERE CONFIRMACIÓN HUMANA.").length,
    ).toBeGreaterThan(0);
  });
  ...
  it("abre evidencia en el visor compartido sin Storage paralelo", async () => {
    ...
    const evidence = await screen.findByRole("button", {
      name: /identificacion-maria.pdf/i,
    });
    await user.click(evidence);
    expect(screen.getByRole("dialog")).toBeInTheDocument();
    expect(
      within(screen.getByRole("dialog")).getByText("identificacion-maria.pdf"),
    ).toBeInTheDocument();
  });

  it("respeta permisos de escritura y datos sensibles", async () => {
    mockApi({ permissions: ["compliance.read"], redacted: true });
    renderAt("/riesgos/revisiones/fixture-uif");
    expect(
      (await screen.findAllByText(/no estos datos sensibles/i)).length,
    ).toBeGreaterThanOrEqual(2);
    expect(
      screen.queryByRole("button", { name: "Reevaluar" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Registrar presentación externa" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("expone contexto PRAVIA IA por revisión real", () => {
    const context = resolveAssistantContext({
      pathname: "/riesgos/revisiones/fixture-uif",
      hash: "",
    });
    expect(context).toMatchObject({
      module: "compliance",
      entityType: "complianceReview",
      entityId: "fixture-uif",
    });
    expect(getAssistantActions(context).map((item) => item.label)).toContain(
      "¿Por qué esta alerta?",
    );
  });
});
