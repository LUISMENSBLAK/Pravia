...
function mockApi(options:{empty?:boolean;fail?:boolean;permissions?:string[]}={}){
  return vi.stubGlobal('fetch',vi.fn(async(input:RequestInfo|URL,init?:RequestInit)=>{const url=String(input);
    ...
    if(url.endsWith('/comparecientes/party-1/extraer-ia')&&init?.method==='POST')return json({success:true,data:{values:{ocupacion:'ARQUITECTA'},proposals:{ocupacion:{valor:'ARQUITECTA',fuente:'INE.pdf',estado:'PENDIENTE_CONFIRMACION'}},conflicts:[],domicilios_detectados:[],identificadores_ine:{cic:'IDMEX183657717',ocr:'0747116375842'}}});
    ...
    if(url.includes('/comparecientes/party-1/documentos/')&&url.endsWith('/vigencia')&&init?.method==='PATCH')return json({success:true,data:{}});
...
describe('Comparecientes client workspace',()=>{
  ...
  it('carga sin metadata, previsualiza contenido PDF/imagen y ofrece descarga y eliminación confirmada',async()=>{mockApi();const user=userEvent.setup();renderPath('/comparecientes/party-1');await screen.findByRole('button',{name:'Previsualizar INE.pdf'});const input=screen.getByLabelText(/Arrastra archivos aquí/) as HTMLInputElement;await user.upload(input,new File(['pdf'],'nuevo.pdf',{type:'application/pdf'}));await waitFor(()=>expect(vi.mocked(fetch).mock.calls.some(([url,init])=>String(url).endsWith('/party-1/documentos')&&init?.method==='POST')).toBe(true));expect(screen.queryByLabelText(/categoría/i)).not.toBeInTheDocument();await user.click(screen.getByRole('button',{name:'Previsualizar INE.pdf'}));const canvas=await screen.findByLabelText('Página 1 de INE.pdf');await waitFor(()=>expect(canvas).toHaveAttribute('data-preview-loaded','true'));await user.click(screen.getByRole('button',{name:'Cerrar vista previa'}));await user.click(screen.getByRole('button',{name:'Previsualizar domicilio.png'}));const image=await screen.findByAltText('Vista previa de domicilio.png');expect(image).toHaveAttribute('src',expect.stringContaining('blob:local-preview-'));fireEvent.load(image);expect(image).toHaveAttribute('data-preview-loaded','true');await user.click(screen.getByRole('button',{name:'Cerrar vista previa'}));await user.click(screen.getByRole('button',{name:'Descargar INE.pdf'}));await waitFor(()=>expect(vi.mocked(fetch).mock.calls.some(([url])=>String(url).includes('/documentos/doc-1/descargar'))).toBe(true));await user.click(screen.getByRole('button',{name:'Eliminar INE.pdf'}));const dialog=screen.getByRole('alertdialog');expect(within(dialog).getByText(/conservará la trazabilidad/)).toBeInTheDocument();await user.click(within(dialog).getByRole('button',{name:'Eliminar documento'}));await waitFor(()=>expect(vi.mocked(fetch).mock.calls.some(([url,init])=>String(url).includes('/documentos/doc-1')&&init?.method==='DELETE')).toBe(true))});

  it('aplica propuesta IA al borrador sin guardarla automáticamente',async()=>{mockApi();const user=userEvent.setup();renderPath('/comparecientes/party-1');await screen.findByRole('button',{name:'Previsualizar INE.pdf'});await user.click(screen.getByRole('button',{name:'Extraer información con IA'}));expect(await screen.findByDisplayValue('ARQUITECTA')).toBeInTheDocument();expect(screen.getByText('Fuente: INE.pdf')).toBeInTheDocument();expect(vi.mocked(fetch).mock.calls.some(([url,init])=>String(url).endsWith('/party-1')&&init?.method==='PATCH')).toBe(false)});

  it('separa CIC y OCR, conserva IDMEX y no selecciona automáticamente cuando existen ambos',async()=>{mockApi();const user=userEvent.setup();renderPath('/comparecientes/party-1');await screen.findByRole('button',{name:'Previsualizar INE.pdf'});await user.click(screen.getByRole('button',{name:'Extraer información con IA'}));const cic=await screen.findByLabelText(/CIC: IDMEX183657717/);const ocr=screen.getByLabelText(/OCR: 0747116375842/);expect(cic).not.toBeChecked();expect(ocr).not.toBeChecked();await user.click(cic);expect(screen.getByLabelText('Folio')).toHaveValue('IDMEX183657717');await user.click(ocr);expect(screen.getByLabelText('Folio')).toHaveValue('0747116375842')});

  it('distingue documentos vigentes e históricos y permite reclasificarlos',async()=>{mockApi();const user=userEvent.setup();renderPath('/comparecientes/party-1');await screen.findByRole('button',{name:'Previsualizar INE.pdf'});expect(screen.getByText('Histórico · 12 KB')).toBeInTheDocument();await user.click(screen.getByRole('button',{name:'Pasar a histórico INE.pdf'}));await waitFor(()=>expect(vi.mocked(fetch).mock.calls.some(([url,init])=>String(url).endsWith('/documentos/doc-1/vigencia')&&init?.method==='PATCH')).toBe(true))});
