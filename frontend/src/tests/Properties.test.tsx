...
describe('PRD-001 ficha maestra inmobiliaria', () => {
  ...
  it('presenta una referencia registral simple como texto humano y preserva JSON estructurado', async () => {
    ...
    expect(await screen.findByDisplayValue('Libro 10, tomo 2, partida 77')).toBeInTheDocument();
    expect(screen.queryByDisplayValue(/"referencia"/)).not.toBeInTheDocument();
    await user.click(screen.getByRole('button', { name: 'Guardar ficha' }));
    expect(mocks.update).toHaveBeenCalledWith('property-1', expect.objectContaining({ datos_registrales: 'Libro 10, tomo 2, partida 77' }));
  });
  ...
  it('requiere selección explícita antes de analizar IA', async () => { const user = userEvent.setup(); renderPath('/predios/property-1'); await screen.findByText('Extracción asistida'); expect(mocks.propose).not.toHaveBeenCalled(); const button = screen.getByRole('button', { name: /Analizar.*fuentes seleccionadas/ }); expect(button).toBeDisabled(); await user.click(screen.getByLabelText('titulo.pdf')); await user.click(screen.getByRole('button', { name: /Analizar 1 fuente seleccionada/ })); await waitFor(() => expect(mocks.propose).toHaveBeenCalledWith('property-1', ['doc-1'])); });
  it('muestra conflicto actual contra propuesto y conserva por defecto', async () => { const user = userEvent.setup(); renderPath('/predios/property-1'); await screen.findByText('Extracción asistida'); await user.click(screen.getByLabelText('titulo.pdf')); await user.click(screen.getByRole('button', { name: /Analizar 1 fuente seleccionada/ })); const dialog = await screen.findByRole('dialog', { name: /Propuesta desde 1 fuente/ }); expect(within(dialog).getByText('FR-001')).toBeInTheDocument(); expect(within(dialog).getAllByText('FR-002').length).toBeGreaterThan(0); expect(within(dialog).getByLabelText('Conservar actual')).toBeChecked(); expect(mocks.applyProposal).not.toHaveBeenCalled(); });
  it('presenta datos registrales y colindancias de IA como texto humano, sin objetos ni JSON técnicos', async () => {
    mocks.propose.mockResolvedValueOnce({
      extraccion_id: 'extract-human', documentos: [{ id: 'doc-1', nombre: 'titulo.pdf' }], alertas: [], conflictos: [], persisted_master: false,
      propuestas: [
        { campo: 'datos_registrales', valor_actual: '{"referencia":"Libro 10, tomo 2"}', valor_propuesto: 'Libro 11, tomo 3', confianza: 'LECTURA_CLARA', fuentes: [] },
        { campo: 'colindancias', valor_actual: '[{"referencia":"Norte","medida":"20","unidad":"m","colindante":"Lote 12"}]', valor_propuesto: '[{"referencia":"Sur","medida":"22","unidad":"m","colindante":"Calle"}]', confianza: 'LECTURA_CLARA', fuentes: [] },
      ],
    });
    ...
    expect(within(dialog).getByText('Libro 10, tomo 2')).toBeInTheDocument();
    expect(within(dialog).getByText('1. Norte · 20 m · con Lote 12')).toBeInTheDocument();
