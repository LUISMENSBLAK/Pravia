...
import { EligibleQuoteSelector } from './components/EligibleQuoteSelector';
...
export function ExpedientesPage() {
  ...
  return <PageContainer title="Expedientes" subtitle="Control y seguimiento integral de operaciones notariales." action={canWrite && <button type="button" className={styles.primaryButton} onClick={showQuoteSelector}><Plus size={18} />Nuevo expediente</button>}>
    ...
    {status === 'ready' && result && <><ExpedienteMetrics metrics={result.metrics} active={values.macrophase} onSelect={selectMetric} />
      {!result.data.length && !Object.values(values).some(Boolean) ? <section className={styles.pageState}><span><FolderOpen /></span><h2>No hay expedientes.</h2><p>Cuando conviertas una cotización elegible aparecerá aquí.</p>{canWrite && <button type="button" className={styles.primaryButton} onClick={showQuoteSelector}><Plus size={18} />Seleccionar cotización</button>}</section> : <section className={styles.listCard} aria-label="Lista de expedientes">{result.data.length ? <><ExpedienteTable items={result.data} values={values} facets={result.facets} onChange={change} /><div className={styles.mobileList}>{result.data.map((item) => <ExpedienteMobileCard key={item.id} item={item} />)}</div></> : <div className={styles.filteredEmpty}><strong>No encontramos expedientes con estos filtros.</strong><button type="button" onClick={clear}>Limpiar filtros</button></div>}<nav className={styles.pagination} aria-label="Paginación de expedientes"><span>Mostrando {result.data.length ? (page - 1) * result.meta.pageSize + 1 : 0} a {Math.min(page * result.meta.pageSize, result.meta.total)} de {result.meta.total}</span><div><button type="button" aria-label="Página anterior" disabled={!result.meta.hasPreviousPage} onClick={() => change('page', String(page - 1))}><ChevronLeft size={17} /></button><b>{page}</b><button type="button" aria-label="Página siguiente" disabled={!result.meta.hasNextPage} onClick={() => change('page', String(page + 1))}><ChevronRight size={17} /></button></div></nav></section>}
    </>}
    {open && <EligibleQuoteSelector onClose={closeQuoteSelector} onConverted={(expediente) => { closeQuoteSelector(); setToast(expediente.idempotent ? `La cotización ya corresponde a ${expediente.numero_pravia}.` : `Expediente ${expediente.numero_pravia} creado correctamente.`); void reload(); navigate(`/expedientes/${expediente.id}`); }} />}
