...
export function ExpedienteTable({ items }: { items: ExpedienteListItem[] }) { return <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Folio</th><th>Acto</th><th>Cliente</th><th>Etapa</th><th>Responsable</th><th>Actualización</th><th>Vulnerable</th><th>Cumplimiento</th><th><span className={styles.srOnly}>Acciones</span></th></tr></thead><tbody>{items.map((item) => <tr key={item.id}>
