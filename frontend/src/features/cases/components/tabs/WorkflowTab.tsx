...
export function WorkflowTab({ expediente, onChanged }: { expediente: ExpedienteDetail; onChanged(): void }) {
  const [saving, setSaving] = useState(false); const [error, setError] = useState(''); const [dialog, setDialog] = useState<'scheduled' | 'effective' | null>(null); const [documentRevision, setDocumentRevision] = useState('');
  const completed = new Map((expediente.etapas || []).map((item: any) => [item.clave_snapshot, item]));
  const stages = expediente.workflow.stages || [];
  const transition = expediente.estatus === 'FIRMA_PROGRAMADA'
    ? expediente.workflow.transitions.find((item) => item.status === 'FIRMADO') || expediente.workflow.transitions[0]
    : expediente.workflow.transitions[0];
  const advance = async (target: ExpedienteTransition, data?: { date: string; place?: string }) => {
    setSaving(true); setError('');
    try {
      await expedientesService.transition(expediente.id, { expected_version: expediente.version, nuevo_estatus: target.status, ...(target.stage?.clave ? { nueva_etapa_clave: target.stage.clave } : {}), ...(target.requires_signature_data && data ? { datos_firma: { fecha_firma: data.date, lugar: data.place || '' } } : {}), ...(target.requires_effective_date && data ? { fecha_efectiva: data.date } : {}), ...(target.status === 'FIRMADO' ? { document_revision: documentRevision } : {}) });
      setDialog(null); onChanged();
    } catch { setError('No fue posible avanzar. Revisa los requisitos pendientes.'); }
    finally { setSaving(false); }
  };
  const startTransition = async () => { if (!transition) return; if (transition.status === 'FIRMADO') { setSaving(true); setError(''); try { const appendix = await expedientesService.documentAppendix(expediente.id); setDocumentRevision(appendix.revision); setDialog('effective'); } catch { setError('No pudimos revisar el apéndice documental antes de firmar.'); } finally { setSaving(false); } return; } if (transition.requires_signature_data) setDialog('scheduled'); else if (transition.requires_effective_date) setDialog('effective'); else void advance(transition); };
...
function TransitionDateDialog({ mode, onClose, onSave, saving }: { mode: 'scheduled' | 'effective'; onClose(): void; onSave(data: { date: string; place?: string }): void; saving: boolean }) {
  ...
  return (
    <div className={styles.dialogBackdrop}>
      <section className={styles.dialog} role="dialog" aria-modal="true" aria-label={scheduled ? 'Programar firma' : 'Registrar firma efectiva'}>
        <header>
          <div>
            <h2>{scheduled ? 'Programar firma' : 'Registrar firma efectiva'}</h2>
            <p>{scheduled
              ? 'Registra la fecha prevista y el lugar.'
              : 'Al confirmar, el apéndice documental revisado quedará congelado. Si la documentación cambió, PRAVIA bloqueará la firma.'}</p>
          </div>
        </header>
        <div className={styles.dialogBody}>
          <label>
            {scheduled ? 'Fecha y hora programada' : 'Fecha y hora efectiva'}
            <input type="datetime-local" value={date} onChange={(event) => setDate(event.target.value)} />
          </label>
          {scheduled && <label>Lugar<input value={place} onChange={(event) => setPlace(event.target.value)} placeholder="Notaría o ubicación" /></label>}
        </div>
        <footer>
          <button type="button" className={styles.secondaryButton} onClick={onClose}>Cancelar</button>
          <button type="button" className={styles.primaryButton} disabled={!date || (scheduled && !place) || saving} onClick={() => onSave({ date, place })}>
            {saving && <LoaderCircle className={styles.spin} size={15} />}
            {scheduled ? 'Guardar programación' : 'Registrar firma y congelar apéndice'}
          </button>
        </footer>
      </section>
    </div>
