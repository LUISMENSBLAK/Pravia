...
import { QuestionnairesTab } from './components/tabs/QuestionnairesTab';
...
import { ExpedienteInformationHeader } from './components/ExpedienteInformationHeader';

type TabKey = ExpedienteSection;
const tabLabels: Record<TabKey, string> = { actos: 'Actos', comparecientes: 'Comparecientes', predios: 'Predios / Inmuebles', documentos: 'Documentos', seguimiento: 'Seguimiento', plantillas: 'Plantillas y formatos', cuestionarios: 'Cuestionarios', presupuesto: 'Presupuesto', proyecto: 'Proyecto', finanzas: 'Finanzas', isr: 'Cálculo ISR', cumplimiento: 'Cumplimiento', actividad: 'Actividad' };
...
export function ExpedienteWorkspace() {
  ...
  useEffect(() => { if (location.hash.slice(1) === 'resumen') navigate({ hash: 'actos' }, { replace: true }); }, [location.hash, navigate]);
  useEffect(() => {
  ...
  return <div className={styles.workspace}><Link className={styles.backLink} to="/expedientes" onClick={(event) => { if (!confirmLeaveBudget()) event.preventDefault(); }}><ArrowLeft size={17} />Volver a expedientes</Link>
    <ExpedienteInformationHeader expediente={expediente} onChanged={() => load()} onAssistant={() => openAssistant({ prefill: '¿Qué falta en este expediente?' })} onSeguimiento={() => { if (confirmLeaveBudget()) navigate('#seguimiento'); }} />
    ...
    <main className={styles.tabContent} role="tabpanel" aria-label={tabs.find((tab) => tab.key === active)?.label}>{active === 'actos' && <ActsTab expediente={expediente} onChanged={() => void load()} />}{active === 'comparecientes' && <PartiesTab expediente={expediente} />}{active === 'predios' && <Suspense fallback={<div role="status">Cargando predios…</div>}><PropertiesTab expediente={expediente} onChanged={() => void load()} /></Suspense>}{active === 'documentos' && <DocumentsTab expediente={expediente} onChanged={() => void load()} />}{active === 'seguimiento' && <WorkflowTab expediente={expediente} onChanged={() => void load()} />}{active === 'plantillas' && <TemplatesFormatsTab expediente={expediente} />}{active === 'cuestionarios' && <QuestionnairesTab expediente={expediente} />}{active === 'presupuesto' && <BudgetTab expedienteId={expediente.id} onDirtyChange={setBudgetDirty} />}{active === 'proyecto' && <ProjectTab expediente={expediente} project={project} onChanged={() => void load()} />}{active === 'finanzas' && (expediente.capabilities.canReadCaseFinance || expediente.capabilities.canReadFinance) && <FinanceTab expediente={expediente} onChanged={() => void load()} />}{active === 'isr' && <ISRTab expedienteId={expediente.id} canWrite={fixtureISR||!user?.permissions||user.permissions.includes('isr.write')} fixture={fixtureISR} />}{active === 'cumplimiento' && canReadCompliance && <ComplianceTab expediente={expediente} />}{active === 'actividad' && <ActivityTab expediente={expediente} fixture={fixtureActivity} />}</main>
  </div>;
}
