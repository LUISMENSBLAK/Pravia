...
import type { ActTypeOption, EligibleQuoteCandidate, ExpedienteAct, ExpedienteActCommand, ExpedienteActPreview, ExpedienteActivityCategory, ExpedienteActivityResponse, ExpedienteArtifacts, ExpedienteBudget, ExpedienteBudgetConcept, ExpedienteDetail, ExpedienteDocumentAppendix, ExpedienteListFilters, ExpedienteListResult, ExpedientePartyCatalogs, ExpedientePartyCommand, ExpedientePartyPreview, ExpedientePartyRelation, ExpedientePartySearchOption, ExpedientePredioCatalogs, ExpedientePredioCommand, ExpedientePredioPreview, ExpedientePredioRelation, ExpedienteQuestionnaireInstance, ExpedienteSeguimiento, PredioSummary, ProjectState, ProjectWorkspace, SeguimientoEstado } from './expedientes.types';
...
const query = (filters: ExpedienteListFilters) => {
  ...
  const values: Record<string, string | number | undefined> = {
    ...
    numero_escritura: filters.deedNumber, fecha_escritura_desde: filters.deedDateFrom, fecha_escritura_hasta: filters.deedDateTo,
...
export const expedientesService = {
  ...
  updateHeader(id: string, input: Record<string, unknown>) { return apiRequest<ExpedienteDetail>(`/expedientes/${encodeURIComponent(id)}`, { method: 'PATCH', body: JSON.stringify(input) }); },
  ...
  ensureQuestionnaires(id: string) { return apiRequest(`/expedientes/${encodeURIComponent(id)}/cuestionarios/asegurar`, { method: 'POST', body: '{}' }); },
  saveQuestionnaire(id: string, input: { assessmentId: string; baseFingerprint: string; answers: Record<string, unknown>; finalize: boolean; idempotencyKey: string }) { return apiRequest(`/expedientes/${encodeURIComponent(id)}/cuestionarios/respuestas`, { method: 'POST', body: JSON.stringify(input) }); },
  ...
  projectWorkspace(id: string, signal?: AbortSignal) { return apiRequest<ProjectWorkspace>(`/expedientes/${encodeURIComponent(id)}/proyecto/workspace`, { signal }); },
  generateProject(id: string, input: { templateVersionId?: string; instructions?: string; sourceDocumentIds?: string[] } = {}) { return apiRequest<{ version: unknown; pending_count: number; generation_observation_count: number; residual_observation_count: number; docx_structural_fidelity: 'PASS'; review_required: boolean }>(`/expedientes/${encodeURIComponent(id)}/proyecto/generar`, { method: 'POST', body: JSON.stringify({ template_version_id: input.templateVersionId || null, instructions: input.instructions?.trim() || null, source_document_ids: input.sourceDocumentIds || [] }) }); },
  generateProjectFromTemplate(id: string, file: File, input: { instructions?: string; sourceDocumentIds?: string[] } = {}) { const body = new FormData(); body.set('file', file); body.set('metadata', JSON.stringify({ instructions: input.instructions?.trim() || null, source_document_ids: input.sourceDocumentIds || [] })); return apiRequest<{ version: unknown; pending_count: number; generation_observation_count: number; residual_observation_count: number; docx_structural_fidelity: 'PASS'; review_required: boolean }>(`/expedientes/${encodeURIComponent(id)}/proyecto/generar-desde-machote`, { method: 'POST', body }); },
  reviewProject(id: string) { return apiRequest(`/expedientes/${encodeURIComponent(id)}/proyecto/analizar-ia`, { method: 'POST', body: JSON.stringify({}) }); },
  downloadProjectReport(id: string, name = 'revision-proyecto.docx') { return downloadBlob(`/expedientes/${encodeURIComponent(id)}/proyecto/reporte-ia/descargar`, name); },
