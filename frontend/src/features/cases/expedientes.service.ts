...
import type { ActTypeOption, EligibleQuoteCandidate, ExpedienteAct, ExpedienteActCommand, ExpedienteActPreview, ExpedienteActivityCategory, ExpedienteActivityResponse, ExpedienteArtifacts, ExpedienteBudget, ExpedienteBudgetConcept, ExpedienteDetail, ExpedienteDocumentAppendix, ExpedienteListFilters, ExpedienteListResult, ExpedientePartyCatalogs, ExpedientePartyCommand, ExpedientePartyPreview, ExpedientePartyRelation, ExpedientePartySearchOption, ExpedientePredioCatalogs, ExpedientePredioCommand, ExpedientePredioPreview, ExpedientePredioRelation, ExpedienteQuestionnaireInstance, ExpedienteSeguimiento, PredioSummary, ProjectGenerationResult, ProjectState, ProjectWorkspace, SeguimientoEstado } from './expedientes.types';
...
export const expedientesService = {
  ...
  generateProject(id: string, input: { templateVersionId?: string; instructions?: string; sourceDocumentIds?: string[] } = {}) { return apiRequest<ProjectGenerationResult>(`/expedientes/${encodeURIComponent(id)}/proyecto/generar`, { method: 'POST', body: JSON.stringify({ template_version_id: input.templateVersionId || null, instructions: input.instructions?.trim() || null, source_document_ids: input.sourceDocumentIds || [], origin: 'UI' }) }); },
  generateProjectFromTemplate(id: string, file: File, input: { instructions?: string; sourceDocumentIds?: string[] } = {}) { const body = new FormData(); body.set('file', file); body.set('metadata', JSON.stringify({ instructions: input.instructions?.trim() || null, source_document_ids: input.sourceDocumentIds || [], origin: 'UI' })); return apiRequest<ProjectGenerationResult>(`/expedientes/${encodeURIComponent(id)}/proyecto/generar-desde-machote`, { method: 'POST', body }); },
  ...
  async projectPreviewUrl(id: string, versionId: string) { return URL.createObjectURL(await apiBlobRequest(`/expedientes/${encodeURIComponent(id)}/proyecto/versions/${encodeURIComponent(versionId)}/visualizar`)); },
