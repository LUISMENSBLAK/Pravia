  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      const updated = await prospectsService.update(prospect.id, {
        ...payload,
        expectedVersion: prospect.version_operativa ?? 0,
      ...
      setErrors({ form: error instanceof ApiError && error.status === 409 ? 'La ficha cambió en otra sesión. Cancela y actualiza la ficha para revisar los cambios.' : 'No pudimos guardar los cambios. Revisa los datos e inténtalo de nuevo.' });
      setSubmitting(false);
    }
  };

  return <section className={styles.detailSection} aria-label="Editar prospecto"><h2>Editar prospecto</h2><p>Actualiza la ficha sin alterar su etapa ni su historial contractual.</p>
    <form id="edit-prospect-form" className={styles.prospectForm} onSubmit={submit} noValidate>
      ...
      <CatalogCombobox label="Etapa documental" value={form.etapa_operativa_codigo ?? ''} options={catalogs.stages} placeholder="Sin etapa documental asignada" error={errors.etapa} onChange={(value) => update('etapa_operativa_codigo', value)} />
      ...
      <div className={styles.fullField}><button className={styles.secondaryButton} type="button" onClick={onClose} disabled={submitting}>Cancelar</button> <button className={styles.primaryButton} type="submit" disabled={submitting}>{submitting && <LoaderCircle className={styles.spin} size={17} />}Guardar cambios</button></div>
    </form>
  </section>;
}
