...
export function NewReviewDialog({ catalogs, onClose, onCreated }: { catalogs:any; onClose:()=>void; onCreated:(id:string)=>void }) {
 ...
 const rules=useMemo(()=>catalogs.reglas.filter((rule:any)=>rule.tipo===draft.tipo),[catalogs.reglas,draft.tipo]);
 const change=(key:string,value:string)=>setDraft(current=>({...current,[key]:value,...(key==='tipo'?{rule_set_id:''}:{})}));
 ...
 return <div className={styles.backdrop} onMouseDown={e=>{if(e.target===e.currentTarget)onClose();}}><section className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="new-review-title"><header><div><small>Nueva evaluación</small><h2 id="new-review-title">Iniciar revisión de cumplimiento</h2></div><button aria-label="Cerrar" onClick={onClose}><X/></button></header><form onSubmit={submit}>
  ...
  <div className={styles.formPair}><label><span>Área</span><select value={draft.tipo} onChange={e=>change('tipo',e.target.value)}><option value="UIF">UIF</option><option value="ISR">ISR</option></select></label><label><span>Fecha de operación</span><input required type="date" value={draft.fecha_operacion} onChange={e=>change('fecha_operacion',e.target.value)}/></label></div>
