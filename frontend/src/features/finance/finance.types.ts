export type FinanceView = 'resumen' | 'movimientos' | 'comprobantes' | 'cuentas' | 'conciliacion' | 'facturacion' | 'cartera';
...
export type FinanceCatalogs = {
  ...
  permisos: { escribir:boolean; aplicar:boolean; conciliar:boolean };
...
export type FinanceMovement = {
  ...
  distribuciones:MovementAllocation[];comprobanteInterno?:{id:string;folio:string;estado:string}|null;
};
export type Paginated<T> = { items:T[];meta:{page:number;pageSize:number;total:number;totalPages:number;agingAvailable?:boolean} };
