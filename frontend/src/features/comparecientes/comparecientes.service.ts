...
export const comparecientesService = {
  ...
  uploadDocument(id: string, file: File, category: string, metadata: { issueDate?: string; expiryDate?: string; notes?: string; vigencia?:'VIGENTE'|'HISTORICO' } = {}) { const body = new FormData(); body.set('file', file); body.set('categoria', category); body.set('vigencia',metadata.vigencia||'VIGENTE'); if (metadata.issueDate) body.set('fecha_emision', metadata.issueDate); if (metadata.expiryDate) body.set('fecha_vencimiento', metadata.expiryDate); if (metadata.notes) body.set('observaciones', metadata.notes); return apiRequest(`/comparecientes/${encodeURIComponent(id)}/documentos`, { method: 'POST', body }); },
  classifyDocument(id:string,documentId:string,vigencia:'VIGENTE'|'HISTORICO'){return apiRequest(`/comparecientes/${encodeURIComponent(id)}/documentos/${encodeURIComponent(documentId)}/vigencia`,{method:'PATCH',body:JSON.stringify({vigencia})})},
  ...
  extractExisting(id: string) { return apiRequest<{ data: { values: Record<string,string>; proposals: Record<string,any>; conflicts: Array<Record<string,any>>; domicilios_detectados: Array<Record<string,any>>; identificadores_ine?:{cic?:string;ocr?:string} } }>(`/comparecientes/${encodeURIComponent(id)}/extraer-ia`, { method: 'POST' }).then(unwrap); },
