...
export function ComparecienteCards({ items }: { items: ComparecienteListItem[] }) {
  return <div className={styles.cardGrid} aria-label="Vista de tarjetas de comparecientes">{items.map((item) => <Link key={item.id} to={`/comparecientes/${item.id}`} className={styles.partyCard} aria-label={`Abrir ficha de ${item.nombre}`}>
    <header><span className={styles.partyAvatar} aria-hidden="true">{item.tipo_persona === 'FISICA' ? <UserRound /> : <Building2 />}<b>{initials(item.nombre)}</b></span><div><span className={styles.typeBadge}>{item.tipo_persona === 'FISICA' ? 'Persona física' : 'Persona moral'}</span><StatusBadge state={item.identidad} /></div></header>
    ...
    {item.documentos.con_observacion > 0 && <p className={styles.observationSignal}>{item.documentos.con_observacion} documento{item.documentos.con_observacion === 1 ? '' : 's'} requiere{item.documentos.con_observacion === 1 ? '' : 'n'} atención</p>}
