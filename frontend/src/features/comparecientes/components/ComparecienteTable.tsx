...
export function ComparecienteTable({ items }: { items: ComparecienteListItem[] }) {
  return <div className={styles.tableWrap}><table className={styles.table}><thead><tr><th>Nombre / Razón social</th><th>Tipo de persona</th><th>RFC</th><th>Expedientes</th><th>Identidad</th><th>Documentos</th><th>Actualización</th><th><span className={styles.srOnly}>Acciones</span></th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><Link className={styles.nameLink} to={`/comparecientes/${item.id}`}>{item.nombre}</Link>{item.curp && <small>CURP · {item.curp}</small>}</td><td><span className={styles.typeBadge}>{item.tipo_persona === 'FISICA' ? 'Física' : 'Moral'}</span></td><td className={!item.rfc ? styles.missing : ''}>{item.rfc || 'Sin RFC'}</td><td><Link className={styles.countLink} to={`/comparecientes/${item.id}#expedientes`}><Folder size={15} />{item.expedientes_vinculados}</Link></td><td><span className={`${styles.stateBadge} ${stateClass(item.identidad)}`}>{identityLabel[item.identidad]}</span></td><td><span className={styles.documentCell}><FileText size={15} />{documentLabel(item)}</span></td><td><time dateTime={item.updated_at}>{formatDate(item.updated_at)}</time></td><td><Link className={styles.rowAction} aria-label={`Abrir ${item.nombre}`} to={`/comparecientes/${item.id}`}><ChevronRight size={18} /></Link></td></tr>)}</tbody></table></div>;
}
