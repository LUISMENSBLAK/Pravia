...
export function AssistantLayer({ mobileSidebarOpen = false }: { mobileSidebarOpen?: boolean }) {
  const { isOpen, closeAssistant } = useAssistant();
  if (!isOpen) return <><AssistantSuggestionBubble /><AssistantLauncher mobileSidebarOpen={mobileSidebarOpen} /></>;
