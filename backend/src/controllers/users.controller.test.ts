...
const db = vi.hoisted(() => ({
  user: { findUnique: vi.fn(), count: vi.fn(), update: vi.fn() }, organizationMembership: { count: vi.fn(), update: vi.fn() }, userInvitation: { findFirst: vi.fn(), create: vi.fn(), count: vi.fn() },
...
const actor: any = { id: '11111111-1111-4111-8111-111111111111', email: 'direccion@pravia.mx', nombre: 'Ana', apellido: 'Ruiz', rol: 'DIRECCION', sessionId: '22222222-2222-4222-8222-222222222222', organizationId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', membershipId: 'membership-a', scope: 'GLOBAL', permissions: ['usuarios.manage'], requiresPasswordChange: false };
...
describe('Administración de usuarios', () => {
  ...
  it('protege al último usuario activo de Dirección', async () => {
    db.user.findUnique.mockResolvedValue({ id: '33333333-3333-4333-8333-333333333333', nombre: 'Última', apellido: 'Dirección', rol: 'DIRECCION', activo: true, organizationMemberships: [{ id: 'member-target', rol: 'DIRECCION', status: 'ACTIVE' }] }); db.organizationMembership.count.mockResolvedValue(1);
  ...
  it('exige confirmación cuando una suspensión impacta trabajo activo', async () => {
    db.user.findUnique.mockResolvedValue({ id: '33333333-3333-4333-8333-333333333333', nombre: 'Mario', apellido: 'Pérez', rol: 'ABOGADO', activo: true, organizationMemberships: [{ id: 'member-target', rol: 'ABOGADO', status: 'ACTIVE' }] });
