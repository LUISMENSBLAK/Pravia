...
describe('reserva concurrente de folio de expediente', () => {
  it('toma el advisory lock anual y considera formatos históricos y nuevos', async () => {
    ...
    const tx = {
      ...
      $queryRaw: vi.fn(async () => { order.push('read'); return [{ numero_pravia: 'EXP-2026-0009' }, { numero_pravia: 'EXP-0012-2026' }, { numero_pravia: 'EXP-2026-INVALIDO' }]; }),
    } as any;
  ...
  it('mantiene una secuencia independiente por año dentro de la transacción', async () => {
    const tx = { $executeRaw: vi.fn().mockResolvedValue(1), $queryRaw: vi.fn().mockResolvedValue([]) } as any;
  ...
  it('no interpreta ni modifica un folio ambiguo', async () => {
    const tx = { $executeRaw: vi.fn().mockResolvedValue(1), $queryRaw: vi.fn().mockResolvedValue([{ numero_pravia: 'EXP/2026/0099' }]) } as any;
