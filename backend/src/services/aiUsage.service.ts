...
type UsageDb = PrismaClient | Prisma.TransactionClient;

export function recordAIUsageInDb(db: UsageDb, metrics: AIUsageMetrics, context: AIUsageContext) {
  ...
  return context.operationId
    ? db.aIUsageLog.upsert({ where: { operation_id: context.operationId }, create: data, update: {} })
    : db.aIUsageLog.create({ data });
}

export async function recordAIUsage(metrics: AIUsageMetrics, context: AIUsageContext) {
  return recordAIUsageInDb(prisma, metrics, context);
}
