  ...
  private async buildDataset(user: User, expedienteId: string) {
    ...
    const expediente = await db.expediente.findFirst({
      ...
      select: {
        ...
        actos: { where: { estatus: 'ACTIVO', removed_at: null }, orderBy: { id: 'asc' }, select: { id: true, updated_at: true, tipo_acto: { select: { id: true, nombre: true } } } },
