...
const cases: Array<[string, () => boolean]> = [
  ...
  ['16 CFG-002 consumido por destino funcional',()=>service().includes('catalogoArtefactoDestino.findMany')&&service().includes('formato_version_id')&&service().includes('FINANZAS_SOLICITUD_PAGO')&&service().includes('FINANZAS_RECIBO_PAGO')],
  ['17 documento interno usa exactamente la versión CFG-002 resuelta',()=>service().includes('renderConfiguredFunctionalDocument')&&service().includes('format_checksum')],
