...
const readFinancial: ToolExecutor = async (db, input) => {
  ...
  const records = await db.expediente.findMany({ where: { archived_at: null, ...expScope, ...(id ? { id } : {}) }, include: { cotizacion: true, movimientosFinancieros: { where: { estatus: { in: ['APLICADO', 'VALIDADO', 'RECIBIDO'] } } } }, orderBy: { updated_at: 'desc' }, take: id ? 1 : 100 });
...
const READ_TOOL_HANDLERS: Partial<Record<AssistantToolName, ToolExecutor>> = {
  ...
  getComplianceSummary: async (db, input) => { const args = input.args || {}; const contextReviewId = input.context?.entity_type === 'complianceReview' ? input.context.entity_id : null; const contextual = contextReviewId ? await db.complianceReview.findFirst({ where: { id: contextReviewId }, select: { expediente_id: true } }) : null; const limit = boundedLimit(args.limit); const id = contextual?.expediente_id || resolveContextId(args, input.context, 'expediente_id', 'expediente'); const exp = await findScopedExpediente(db, input.user, id); const reviews = await db.complianceReview.findMany({ where: { expediente_id: id, ...(contextReviewId ? { id: contextReviewId } : {}) }, select: { id: true, tipo: true, estatus: true, rule_version_snapshot: true, rule_snapshot: true, master_snapshot: true, resultado_json: true, explicacion: true, snapshot_captured_at: true, updated_at: true, evidencias: { select: { id: true, tipo_evidencia: true, documento: { select: { id: true, nombre_original: true, tipo: true } } } } }, orderBy: { updated_at: 'desc' }, take: limit }); return { data: { expediente_id: id, folio: exp.numero_pravia, revisiones: reviews, limitaciones: ['La explicación asiste al revisor y no constituye dictamen legal.', 'No existe screening PEP externo ni evidencia de presentación de avisos en este módulo.'] }, provenance: reviews.length ? reviews.map((review) => source('ComplianceReview', review.id, `${review.tipo} · ${review.rule_version_snapshot}`, `/riesgos/revisiones/${review.id}`)) : [source('Expediente', exp.id, exp.numero_pravia, `/expedientes/${exp.id}`)], truncated: reviews.length === limit }; },
